...
[`@xelly/xelly.js` SDK](https://github.com/xelly-games/xelly.js) via
a few key `xel.**` utility functions...
...
export const install: XellyInstallFunction
    ...
    skull.graphics.use(xel.graphics.fromSpriteArray(SkullSprite));
    engine.add(skull);
}
```
...
```typescript title="main.ts"
    ...
    const skullGraphic = xel.graphics.fromSpriteArray(SkullSprite);
